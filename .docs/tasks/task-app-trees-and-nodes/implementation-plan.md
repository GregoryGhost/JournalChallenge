# Implementation Plan: Application Logic (Trees & Nodes)

This task focuses on the core hierarchical tree management logic and unit testing within the **Application Layer**, ensuring sibling name uniqueness and independent tree isolation as required by the specifications.

## 1. Research & Scaffolding
- **Audit Context**: Review `JournalChallenge.Application.Core` for the existing `ICommand`, `IQuery`, and `Result` interfaces.
- **Error Mapping Strategy**:
    ...
    - This allows the Application layer to remain "Pure" (using Results) while satisfying the specified Exception-based response requirements.
- **Define DTOs (Application Layer)**:
    - `NodeDto`: Recursive model with `long Id`, `string Name`, and `IEnumerable<NodeDto> Children`. Matches the `MNode` schema.

## 2. Feature: Get Entire Tree (`GetTreeQuery`)
- **Objective**: Retrieve the full hierarchy of a specified tree by its unique name.
- **Logic**:
...
- **Outcome**: A structured `NodeDto` representing the entire tree as defined in the `MNode` schema.

## 3. Feature: Create Node (`CreateNodeCommand`)
- **Objective**: Add a new node to a tree, ensuring it follows the hierarchy and uniqueness rules.
- **Logic**:
...
- **Error Handling**: Throw or return a `SecureException` if sibling names conflict, the parent ID is invalid, or a second root node is attempted.

## 4. Feature: Rename Node (`RenameNodeCommand`)
- **Objective**: Change an existing node's name while maintaining tree integrity.
- **Logic**:
...
## 5. Feature: Delete Node (`DeleteNodeCommand`)
- **Objective**: Recursively delete a node and all its child nodes based on the forced deletion flag.
- **Logic**:
...
## 6. Unit Testing (Application Logic)
- **Objective**: Verify the behavioral correctness of the application services and commands.
- **Test Cases**:
...
## 7. Mismatches & Ambiguities
- **Delete Logic Contradiction**: The Swagger definition specifies "Delete a node and all its descendants," but the `SecureException` example in the PDF specifies "You have to delete all children nodes first."

## 8. TODO: Alignment Fixes
